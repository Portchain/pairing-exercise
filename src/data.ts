...
class Contacts {
  private records: Record<string, ContactRecord> = {};

  constructor(records: ContactMutation[] = []) {
    for (const record of records) {
      const id = Math.random().toString(36).substring(2, 9);
      this.records[id] = { ...record, id, createdAt: new Date().toISOString() };
    }
  }
